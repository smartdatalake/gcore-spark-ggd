...
case class PeopleGraph(spark: SparkSession) extends SparkGraph {
  ...
  val testOptional = Person(id=105, name= "testOptional1", employer = "")
  val testOptional2 = Person(id=106,name="testOptional2", employer = "")
  ...
  override def vertexData: Seq[Table[DataFrame]] =
    Seq(
      Table(Label("Person"), Seq(frankCwi, frankMit, alice, celine, john, testOptional, testOptional2).toDF()),
