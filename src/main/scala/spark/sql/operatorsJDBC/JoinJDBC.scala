...
import spark.sql.SqlQuery.mergeSchemas
import spark.sql.operators.SqlBindingTableMetadata

/** A generic join of two relations: [[lhs]] JOIN [[rhs]]. */
abstract class JoinJDBC(lhs: TargetTreeNode, rhs: TargetTreeNode) extends target_api.Join(lhs, rhs) {
  ...
  val lhsSchemaJDBC: StructType = lhsBtable.getbtableJDBCSchema()
  val rhsSchemaJDBC: StructType = rhsBtable.getbtableJDBCSchema()
  ...
  override val bindingTable: BindingTableMetadata = {
    ...
    val mergedSchemas : StructType = mergeSchemas(lhsSchemaJDBC,rhsSchemaJDBC)

    val joinQuery: String =
      s"""
      SELECT $commonCols FROM (${lhsBtable.btable.resQuery}) AS $lhsAlias
      $joinTypeSql (${rhsBtable.btable.resQuery}) AS $rhsAlias
      $joinCondition"""
  ...
  def commonCols : String ={
    ...
    val leftTable = s"${lhsSchemaJDBC.map(f => s"$lhsAlias.${f.name}").mkString(" , ")}"
    //val rightTable =  s"${rhsSchema.diff(lhsSchema).map(f => s"$rhsAlias.`${f.name}`").mkString(" , ")}"
    val rightTable =  s"${rhsSchemaJDBC.diff(lhsSchemaJDBC).map(f => s"$rhsAlias.${f.name}").mkString(" , ")}"
