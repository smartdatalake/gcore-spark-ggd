...
import spark.sql.SqlQuery.{expandExpression, mergeSchemas, expandExpressionJDBC}
import spark.sql.operators._
import spark.sql.{operators => sql}
import spark.sql.{operatorsJDBC => sqlJDBC}
...
case class SqlJDBCPlanner(compileContext: CompileContext, con: Connection) extends TargetPlanner {
  val SOLUTION_IDENTIFICATOR: String = "sid"
  ...
  def selectMatchData(bt: DataFrame, selectClause: AlgebraTreeNode): DataFrame = {
    var result = sparkSession.emptyDataFrame
    var selectVar: Map[String,String] = selectClause.asInstanceOf[SelectClause].map
    var prefixesString: ArrayBuffer[String] = ArrayBuffer[String]() //list of variables
    var columnsString: ArrayBuffer[String] = ArrayBuffer[String]()
    if(selectVar.head._1 == "*"){
      return bt
    }
    for(pair <- selectVar){
      if(pair._2 == ""){
        prefixesString += pair._1
      }else{
        columnsString += (pair._1 + '$' + pair._2)
      }
    }
    val prefixes = colNameStartsWith(prefixesString.toSeq, bt)
    val cols = prefixes ++ columnsString
    result = bt.select(cols.map(bt(_)): _*)
    //bt.columns.foreach(str => println(str))
    return result
  }

  def constructSelectGraph(bt: DataFrame): PathPropertyGraph = {
    val graph : SparkGraph = new SparkGraph {
      override var graphName: String = randomString(length = GRAPH_NAME_LENGTH)

      override def vertexData: Seq[Table[DataFrame]] = Seq(Table(Label("Result"), bt))
  ...
  def colNameStartsWith(prefixes: Seq[String], df:DataFrame) : Seq[String] ={
    // Find out if given column name matches any of the provided prefixes
    def colNameStartsWith: String => Boolean = (colName: String) =>
      prefixes.map(prefix => colName.startsWith(prefix)).reduce(_ || _)
    // Filter columns list by checking against given prefixes sequence
    val columns = df.columns.filter(colNameStartsWith)
    return columns
  }

  def unionGraph(resultGraph: PathPropertyGraph, graphUnion: Seq[AlgebraTreeNode]) : PathPropertyGraph = {
    println("Union based on id - Only one graph for union (check spoofax parser)")
    //TODO works only if the StorageType is DataFrame
    val unionGraphNames: Seq[String] = graphUnion.map(_.asInstanceOf[NamedGraph].graphName)
    val unionGraph : PathPropertyGraph = compileContext.catalog.graph(unionGraphNames.head)
    val eRestrictions: LabelRestrictionMap = unionGraph.edgeRestrictions.union(resultGraph.edgeRestrictions)
    val sPathRestrictions: LabelRestrictionMap = unionGraph.storedPathRestrictions.union(resultGraph.storedPathRestrictions)
    val vertex: Seq[Table[DataFrame]] = unionData(resultGraph.vertexData.asInstanceOf[Seq[Table[DataFrame]]], unionGraph.vertexData.asInstanceOf[Seq[Table[DataFrame]]])
    val edges: Seq[Table[DataFrame]] = unionData(resultGraph.edgeData.asInstanceOf[Seq[Table[DataFrame]]], unionGraph.edgeData.asInstanceOf[Seq[Table[DataFrame]]])
    val path : Seq[Table[DataFrame]] = unionData(resultGraph.pathData.asInstanceOf[Seq[Table[DataFrame]]], unionGraph.pathData.asInstanceOf[Seq[Table[DataFrame]]])
    //TODO schema only change columns when the one that has the biggest number of columns is at first (folLeft in union function)
    val vSchema: EntitySchema = resultGraph.vertexSchema.union(unionGraph.vertexSchema)
    val pSchema: EntitySchema = resultGraph.pathSchema.union(unionGraph.pathSchema)
    val eSchema: EntitySchema = resultGraph.edgeSchema.union(unionGraph.edgeSchema)
    //val header = vertexData.head.keys.toList
    //          val schema = StructType(header.map(fieldName => StructField(fieldName, StringType, true)))
    return new SparkGraph {
      override var graphName: String = resultGraph.graphName

      override type StorageType = DataFrame

      override def vertexData: Seq[Table[DataFrame]] = vertex

      override def edgeData: Seq[Table[DataFrame]] = edges

      override def pathData: Seq[Table[DataFrame]] = path

      override def vertexSchema: EntitySchema = vSchema

      override def pathSchema: EntitySchema = pSchema

      override def edgeSchema: EntitySchema = eSchema

      override def edgeRestrictions: LabelRestrictionMap = eRestrictions

      override def storedPathRestrictions: LabelRestrictionMap = sPathRestrictions
    }
  }

  def unionData(result: Seq[Table[DataFrame]], union: Seq[Table[DataFrame]]) : Seq[Table[DataFrame]] = {
    //println("Union data function ok!!!")
    //val resultTableNames: Seq[String] = result.map(_.name.value)
    val unionTableNames: Seq[String] = union.map(_.name.value)
    val tempTableArray : ArrayBuffer[Table[DataFrame]] = new ArrayBuffer[Table[DataFrame]]()
    for(tableResult <- result){
      if(unionTableNames.contains(tableResult.name.value)){
        //println("entrou no if")
        //join table
        val d: DataFrame = mergeDF(tableResult.data, getTableData(tableResult.name, union), "id")//tableResult.data.join(getTableData(tableResult.name), "id", "fullouter")
        //println(tableResult.data.show(3))
        //val d: DataFrame = tableResult.data.join(getTableData(tableResult.name, union), Seq("id"), "fullouter")
        // print(d.show(2))
        tempTableArray += Table(tableResult.name, d)
      }else{
        //create table
        tempTableArray += tableResult
      }
    }
    return tempTableArray.toSeq
  }

  def getTableData(label: Label, seq: Seq[Table[DataFrame]]) : DataFrame = {
    for(table <- seq){
      if(table.name == label) return table.data
    }
  ...
  : target.VertexScan = sqlJDBC.VertexScanJDBC(vertexRelation, graph, catalog)

  override def planEdgeScan(edgeRelation: EdgeRelation, graph: Graph, catalog: Catalog)
  : target.EdgeScan = sqlJDBC.EdgeScanJDBC(edgeRelation, graph, catalog)
  ...
  override def planJoin(joinOp: JoinLike): target.Join = {
    ...
    joinOp match {
      case _: algebra.operators.InnerJoin => sqlJDBC.InnerJoinJDBC(lhs, rhs)
      case _: algebra.operators.CrossJoin => sqlJDBC.CrossJoinJDBC(lhs, rhs)
      case _: algebra.operators.LeftOuterJoin => sqlJDBC.LeftOuterJoinJDBC(lhs, rhs)
    }
  }

  override def planSelect(selectOp: algebra.operators.Select): target.Select =
    sqlJDBC.SelectJDBC(selectOp.children.head.asInstanceOf[TargetTreeNode], selectOp.expr)
  ...
  private def solveBtableOps(relation: AlgebraTreeNode): DataFrame = {
    ...
    val data: DataFrame = btableMetadata.solveBtableOps(sparkSession, connection)
    data
  }
  ...
  private def createEntity(reference: Reference,
    ...
    //val labelsTemp = entityDf.select(labelColSelect).distinct().collect().map(_(0)).toList
    val labels = entityDf.select(labelColSelect).distinct().map(_.getString(0)).collect.toList
