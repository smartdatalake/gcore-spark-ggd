...
case class SpoofaxParser(context: ParseContext) extends ParseStage {
  ...
  override def parse(query: String): AlgebraTreeNode = {
    ...
    val algebraTree: AlgebraTreeNode = AlgebraTreeBuilder build rewriteParseTree //problem here for construct clauses
    ...
    case select: SelectQuery =>
      select.checkWithContext(QueryContext(context.catalog))
      select
    case union: UnionQuery =>
      union.checkWithContext(QueryContext(context.catalog))
      union
    }
  }
}
