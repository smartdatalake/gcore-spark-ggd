...
object ConstructTreeBuilder {
  ...
  def extractConstructClause(from: SpoofaxBaseTreeNode): ConstructClause = {
    ...
    constructs.foreach( constructPattern =>{
      val constructExp = constructPattern.children.head //ok! começa em construct pattern
  ...
  : Seq[ConnectionConstruct] = {
    from match {
      // Vertex
      case Seq(vertex) if vertex.name == "Vertex" => {
        Seq(extractVertexConstruct(vertex))
      }

      // Vertex - EdgeVertexConstructPattern - ...
      case Seq(vertex, connections@_*) if vertex.name == "Vertex" =>{
        from.head.printTree()
        //TODO: if provisorio por causa do None parseado de forma diferente na nova versão do parser --> nunca entra no primeiro case dessa forma
        if(connections.head.name == "None"){
          return Seq(extractVertexConstruct(vertex))
        }
  ...
  private def extractVertexConstruct(from: SpoofaxBaseTreeNode): SingleEndpointConstruct = {
    ...
    val vertexConstruct = VertexConstruct(
