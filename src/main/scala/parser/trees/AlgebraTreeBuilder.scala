...
import parser.trees.QueryTreeBuilder.{extractQueryClause, extractSelectQueryClause, extractUnionQueryClause}
  ...
  override def build(from: SpoofaxBaseTreeNode): AlgebraTreeNode = {
    ...
    from.name match {
      ...
      case "TabularGraphQuery" =>
        val query: SelectQuery = extractSelectQueryClause(from)
        query
      case "Union" =>
        val query: UnionQuery = extractUnionQueryClause(from)
        query
    }
  }
}
