...
object MatchTreeBuilder {
  ...
  def extractMatchClause(from: SpoofaxBaseTreeNode): MatchClause = {
    ...
    val nonOptMatches = extractCondMatchClause(fullGraphPatternCondition)
    val whereClause = nonOptMatches.where

    //TODO location parsing syntax a little bit confusing, recheck parsing for ON keyword
    //THIS WORKS: SELECT * MATCH (n:Person) ON people_graph WHERE n.name = 'Joe' OPTIONAL (n)-[:WorksAt]->(a) ON people_graph
    /*MatchClause(
  ...
  private def extractCondMatchClause(from: SpoofaxBaseTreeNode): CondMatchClause = {
    println("Extract cond match clause")
    from.printTree()
    from.name match {
  ...
  private def extractOptionalMatches(from: SpoofaxBaseTreeNode): Seq[CondMatchClause] = {
    println("OPTIONAL MATCHES" + from.name)
    from.name match {
      case "None" => List.empty
      // Some(OptionalClause(...))
      case "Some" => extractOptionalMatches(from.children.head)
      case "OptionalClause" =>{ //TODO optional clause is a list create extract optional to connect
        //OptionalClause.children = Seq[Optional]
        extractOptionalMatches(from.children.head)
        //from.children.map(optional => {
        //  extractOptionalMatches(from = optional.children.head)
        //})
      }
      case "Optional" => {
        from.children.map(optional => {
  ...
  private def extractWhere(from: SpoofaxBaseTreeNode): AlgebraExpression = {
    from.name match {
      ...
      //case "Optional" => extractWhere(from.children.head) //TODO use where for now
