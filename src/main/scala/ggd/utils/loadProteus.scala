...
class loadProteus(gcoreRunner: GcoreRunner) {
  var con: Connection = null
  var user: String = ""
  var password: String = ""

  def readTableSparkJDBC(tableName: String) : DataFrame = {
    gcoreRunner.sparkSession.read.format("jdbc")
      .option("url", "jdbc:avatica:remote:url=http://diascld32.iccluster.epfl.ch:18010;serialization=PROTOBUF")
  ...
  def openConnection() : Unit = {
    ...
    val url:String ="jdbc:avatica:remote:url=http://diascld32.iccluster.epfl.ch:18010;serialization=PROTOBUF"
    val userIn:String = scala.io.StdIn.readLine("username=>: ")
    user = userIn
    val pwdIn:String = scala.io.StdIn.readLine("pwd=>: ")
    password = pwdIn
    con=DriverManager.getConnection(url,userIn,pwdIn);
  }
  ...
  def loadGraph(configPath: String) : Unit = {
    ...
    val graph = loadGraphProteus(jsonConfig, graphSource)
  ...
  def loadSparkJDBC(configPath: String) : Unit = {
    ...
    val cat: SparkCatalog = new SparkCatalog(gcoreRunner.sparkSession)
    val jsonConfig = GraphSource.parseJsonConfig(Paths.get(configPath))
    val graph = loadGraphProteusSparkJDBC(jsonConfig, graphSource)
    val name = graph.graphName
    println("Graph name: " + graph.graphName)
    println(graph.schemaString)
    cat.registerGraph(graph)
  ...
  def loadGraphProteusSparkJDBC(config: GraphJsonConfig, source: GraphSource): SparkGraph = {
  ...
  def loadGraphProteus(config: GraphJsonConfig, graphSource: GraphSource) : SparkGraph = {
    val edgeLabels = config.edgeLabels
    val vertexLabels = config.vertexLabels
    //val pathLabels = config.pathLabels - for now no paths just vertex and edges
    var vertices = Seq[Any]()
