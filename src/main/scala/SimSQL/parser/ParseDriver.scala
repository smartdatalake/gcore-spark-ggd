 ...
 */
abstract class AbstractSimSqlParser(conf: SQLConf) extends ParserInterface with Logging {
  ...
  override def parseFunctionIdentifier(sqlText: String): FunctionIdentifier = parse(sqlText) { parser =>
  ...
  protected def astBuilder: SimAstBuilder
 ...
 */
class SimCatalystSqlParser(conf: SQLConf) extends AbstractSimSqlParser(conf) {
  val astBuilder = new SimAstBuilder(conf)
}

/** For test-only. */
object SimCatalystSqlParser extends AbstractSimSqlParser(SQLConf.get) {
  val astBuilder = new SimAstBuilder(SQLConf.get)
  println("Sim Catalyst SQL Parser!!!")
}
...
class ParseException(
  ...
  def this(message: String, ctx: ParserRuleContext) = this(Option(ParserUtils.command(ctx)),
    message,
  ...
  def withCommand(cmd: String) = new ParseException(Option(cmd), message, start, stop)
}
