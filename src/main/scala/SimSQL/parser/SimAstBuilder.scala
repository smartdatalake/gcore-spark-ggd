...
import SimSQL.logicalPlan.{EditSimilarity, JaccardSimilarity, Similarity, SimilarityJoin}
...
import SimSQL.parser.SqlBaseBaseVisitor._
import SimSQL.parser.SqlBaseParser._
import org.apache.spark.sql.catalyst.parser.ParserUtils._



class SimAstBuilder(conf: SQLConf) extends SqlBaseBaseVisitor[AnyRef] with Logging{
  import org.apache.spark.sql.catalyst.parser.ParserUtils._
  ...
  protected def toAttributes(struct: StructType): Seq[AttributeReference] =
    ...
    specType match {
      case SqlBaseParser.MAP | SqlBaseParser.REDUCE | SqlBaseParser.TRANSFORM =>
        ...
        val (attributes, schemaLess) = if (colTypeList != null) {
          ...
          (toAttributes(createSchema(colTypeList)), false)
    ...
    ctx.joinRelation.asScala.foldLeft(base) { (left, join) =>
      withOrigin(join) {
        val baseJoinType = join.joinType match {
          ...
          //case jt if jt.SIMILARITY != null => Similarity
          case _ => Inner
        }
        val simJoin = join.joinType() match {
          case jt if jt.SIMILARITY != NULL => Similarity
          case _ => null
        }
        //println(baseJoinType.toString)
        // Resolve the join type and join condition
        /*val (joinType, condition) = Option(join.joinCriteria) match {
          case Some(c) if c.USING() != null && baseJoinType == SimilarityJoin =>
            SimilarityJoin(left, plan(join.right), c.simExpression)
        ...
        Option(join.joinCriteria) match { //update here
         case Some(c) if c.USING != null && c.simExpression != null && simJoin != null => {
            //SimilarityJoin(left, plan(join.right), Option(expression(c.simExpression)), SimJoin)
           println("HERE SIMILARITY JOIN")
          // println(c.simExpression.identifier(0).toString)
          // println(left.toString())
           SimilarityJoin(left, plan(join.right), simJoin, Option(expression(c.simExpression)))
           //SimilarityJoinType(left, plan(join.right), Similarity, Option(expression(c.simExpression)))
         }
          //  SpatialJoin(left, plan(join.right), KNNJoin, Option(expression(c.booleanExpression)))
          case Some(c) if c.USING != null => {
            println("HERE PARSER USING")
            Join(left, plan(join.right), UsingJoin(baseJoinType, c.identifier.asScala.map(_.getText)), None)
          }
          case Some(c) if c.booleanExpression != null => {
            println("HERE PARSER BOOLEAN")
  ...
  protected def RandomSampler_roundingEpsilon = 1e-6
  ...
  private def withSample(ctx: SampleContext, query: LogicalPlan): LogicalPlan = withOrigin(ctx) {
    ...
    def sample(fraction: Double): Sample = {
      ...
      // adjust the fraction.
      val eps = RandomSampler_roundingEpsilon
  ...
  /**
   * Similarity Join Plan
   */
  override def visitSimExpression(ctx: SimExpressionContext): Expression = withOrigin(ctx){
    //println("visitSimExpression")
    //println(ctx.simFunction)
    val operator = ctx.comparisonOperator().getChild(0).asInstanceOf[TerminalNode]
    val operatorLiteral: String = operator.getSymbol.getType match {
   ...
   // val e = expression(ctx)
    /*val numericLiteral = ctx.number.getType match {
      case SqlBaseParser.DoubleLiteralContext => visitDoubleLiteral(ctx.number.asInstanceOf[SqlBaseParser.DoubleLiteralContext])
      case SqlBaseParser.IntegerLiteralContext => visitIntegerLiteral(ctx.number)
      case SqlBaseParser.DecimalLiteralContext => visitDecimalLiteral(ctx.number)
    }*/
    val numericLiteral = Literal(BigDecimal(ctx.number.getText).underlying())
    println("ctx sim expression:" + ctx.simFunction.getText)
    ctx.simFunction.getText.toLowerCase() match {
      case "jaccardsimilarity" =>{
        //println(" JACCARD SIM AST")
        return JaccardSimilarity(expression(ctx.primaryExpression(0)), expression(ctx.primaryExpression(1)), Literal(operatorLiteral), numericLiteral)
      }
      case "editsimilarity" =>
        return EditSimilarity(expression(ctx.primaryExpression(0)), expression(ctx.primaryExpression(1)), Literal(operatorLiteral), numericLiteral)
    }
  }
