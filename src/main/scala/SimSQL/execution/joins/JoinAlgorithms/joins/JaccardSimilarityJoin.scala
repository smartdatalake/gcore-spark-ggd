...
import SimSQL.execution.index.{IPartition, JaccardIndex}
 ...
 */
case class JaccardSimilarityJoinDima(leftKeys: Expression,
  ...
  final val num_partitions = 30//sqlContext.conf.numSimilarityPartitions
  //10240
  final val threshold = thresh.toString.toDouble
  final val alpha = 0.95//sqlContext.conf.similarityMultigroupThreshold
  final val topDegree = 0//sqlContext.conf.similarityBalanceTopDegree
  final val abandonNum = 3//sqlContext.conf.similarityFrequencyAbandonNum
  final val partitionNumToBeSent = 10//sqlContext.conf.partitionNumToBeSent
  final val weight = Some("0").get.split(",").map(x => x.toInt)
  final val splitToken = " "
  ...
  private def partition_r(
    ...
    var result = ArrayBuffer[(Array[(Array[Int], Array[Boolean])],
      Array[(Int, Boolean, Array[Boolean],
        Boolean,
        Int)])]()
    val ss = ss1.split(splitToken).filter(x => x.length > 0)
    val s = ss.size
  ...
  private def sortByValue(x: String): String = {
    sort(x.split(splitToken)).reduce(_ + splitToken + _)
  }
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    /*val left_rdd = leftRDD.filter(pos => pos._1 match{
      case Some(f) => true
      case None => false
    })
    val right_rdd = rightRDD.filter(pos => pos._1 match{
      case Some(f) => true
      case None => false
    })*/

    val rdd1 = left_rdd
      .map(x => (x._1.split(splitToken).filter(_.size > 0).size))
    ...
    query_rdd_partitioned.zipPartitions(index_rdd_indexed, true) {
      (leftIter, rightIter) => {
        ...
        //ans.map(x => new JoinedRow(x._2, x._1).asInstanceOf[InternalRow]).iterator
        ans.map(x => {
          val joinedRow = new JoinedRow()
          joinedRow(x._2, x._1).asInstanceOf[InternalRow].copy()
        }).iterator
      }
    }
  }
