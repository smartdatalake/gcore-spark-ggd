...
case class AlgebraRewriter(context: AlgebraContext) extends RewriteStage {
  ...
  override def rewrite(tree: AlgebraTreeNode): AlgebraTreeNode = {
    ...
    //println("group constructs tree"+ groupConstructs.treeString())
 ...
 /* override def rewriteSelect(tree: AlgebraTreeNode) : AlgebraTreeNode = {
    // Algebra tree analysis.
    val bindingToGraph = MapBindingToGraph(context) mapBindingToGraph tree
    val bindingContext = ExtractReferenceTuples extractReferenceTuples tree
    ...
    //SelectClause rewrite
    val selectTable = SelectClauseToSelectTable(enrichedContext) rewriteTree matchesToAlgebra

    //Query rewrite
    val createTable = BasicQueriesToGraphs rewriteTree selectTable

    logger.info("\n{}", createTable.treeString())
    //return null
    createTable
  }*/
